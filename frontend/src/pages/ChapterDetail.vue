    ...
    <section class="hero-panel overflow-hidden rounded-[32px] px-6 py-8 sm:px-8 lg:px-10">
      <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="metric-card"><span class="metric-label">{{ t('findings') }}</span><strong class="metric-value">{{ chapterFindingsCount }}</strong></div>
          <div class="metric-card"><span class="metric-label">{{ t('conflicts') }}</span><strong class="metric-value">{{ conflicts.length }}</strong></div>
        </div>
      </div>
    </section>

    <div v-if="loading" class="glass-panel p-6">{{ t('chapterLoading') }}</div>
    ...
    <section v-else class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      <div class="space-y-6">
        <article class="glass-panel p-6">
          <div class="section-head">
            <div>
               <p class="section-kicker">{{ t('chapterContent') }}</p>
               <h2 class="section-title">{{ t('chapterPreview') }}</h2>
             </div>
           </div>
          <p class="mt-5 text-sm leading-7 text-[var(--ink-soft)]">{{ chapter.summary || chapter.content?.slice(0, 600) || t('noContentAvailable') }}</p>
        </article>

        <article class="glass-panel p-6">
          <div class="section-head">
            <div>
               <p class="section-kicker">{{ t('linkedFindings') }}</p>
               <h2 class="section-title">{{ t('linkedFindingsTitle') }}</h2>
             </div>
           </div>
          <div class="mt-5 space-y-4">
            <div v-for="group in findings" :key="group.agent_name" class="finding-card" :style="cardStyle(group.agent_name)">
              <div class="flex items-center justify-between gap-3">
                ...
                <span class="tag">{{ group.findings.length }} {{ t('issues') }}</span>
              </div>
              ...
              <div class="mt-4">
                <p class="text-xs uppercase tracking-[0.14em] text-[var(--ink-muted)]">{{ t('suggestions') }}</p>
            ...
            <div v-if="!findings.length" class="empty-state">{{ t('noChapterFindings') }}</div>
          </div>
        </article>
      </div>
      ...
      <div class="space-y-6">
        <article class="glass-panel p-6">
          <div class="section-head">
            <div>
               <p class="section-kicker">{{ t('conflictContext') }}</p>
               <h2 class="section-title">{{ t('chapterConflictTitle') }}</h2>
             </div>
           </div>
          <div class="mt-5 space-y-4">
            ...
            <div v-if="!conflicts.length" class="empty-state">{{ t('noChapterConflict') }}</div>
          </div>
        </article>
        ...
        <article class="glass-panel p-6">
          <div class="section-head">
           ...
           <div class="mt-5 space-y-3">
              ...
              <button class="action-button btn-light w-full" @click="router.push(`/project/${projectId}/report`)">{{ t('viewFinalReport') }}</button>
            </div>
          </article>
      </div>
    </section>
...
const chapterTitle = computed(() => chapter.value.chapterTitle ? `Chapter ${chapter.value.chapterNo}: ${chapter.value.chapterTitle}` : t('chapterDetailFallback'));
...
onMounted(async () => {
  try {
    ...
    error.value = err?.response?.data?.error || t('chapterDetailLoadFailed');
  } finally {
    loading.value = false;
  }
});
